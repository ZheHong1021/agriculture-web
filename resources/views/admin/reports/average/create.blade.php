...
<nav>
  <ol id="breadcrumbWhite" class="breadcrumb">
    ...
    <li class="breadcrumb-item active" aria-current="page">新增全部測點平均值</li>
  </ol>
</nav>
<form method="POST" action="{{route('admin.report.average.store')}}" enctype="multipart/form-data">
  <div class="list-session col-12">
    ...
    <div class="form-inline mb-4 col-12 col-md-4">
      <label>n = </label>
      <input class="form-control ml-2" type="number" name="n" min="0" placeholder="請輸入全部測點平均值的n (n=?)" value="{{ old('n') }}" required>
    </div>
    @if($errors->has('n'))
        ...
        @for($i = 0 ; $i < 2; $i++)
          <tr>
            @foreach($averageColumns as $averageColumn)
              <td><input type="text" name="average[{{$i}}][{{$averageColumn}}]}" placeholder="{{ $averageChineseColumns[$loop->index] }}" class="form-control" value="{{ old( 'average.'.$i.'.'.$averageColumn ) }}" required></td>
            @endforeach
          </tr>
        @endfor  
      </tbody>
